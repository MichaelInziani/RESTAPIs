...
const yourBearerToken = "f7f6e040-044f-4c3b-b5d8-2ea51376b268";
const config = {
...
app.post("/get-secret", async (req, res) => {
  ...
  try {
    ...
    res.render("index.ejs", { content: JSON.stringify(result.data) });
      ...
      const error = JSON.stringify(error.response.data);
...
app.post("/delete-secret", async (req, res) => {
  ...
  try{
      const response = await axios.delete(API_URL + "/secrets/"+searchId, req.body, config);
      const result = JSON.stringify(response.data);
      res.render("index.ejs", {content:result});
  }catch (error) {
      const error = JSON.stringify(error.response.data);
